...
func TestExecuteSafeCommand_Allowlist(t *testing.T) {
	...
	_, err := ExecuteSafeCommand("test-req", "rm", []string{"-rf", "/"})
	if err == nil {
	...
	_, err = ExecuteSafeCommand("test-req", "echo", []string{"hello"})
...
func TestExecuteSafeCommand_Sanitization(t *testing.T) {
	// 1. Shell characters
	_, err := ExecuteSafeCommand("test-req", "echo", []string{"hello", "&&", "ls"})
	if err == nil || !strings.Contains(err.Error(), "illegal characters") {
		t.Errorf("Expected illegal characters error for '&&', got: %v", err)
	}

	_, err = ExecuteSafeCommand("test-req", "echo", []string{"hi", ">", "file.txt"})
	if err == nil || !strings.Contains(err.Error(), "illegal characters") {
		t.Errorf("Expected illegal characters error for '>', got: %v", err)
	}

	// 2. Path Traversal
	_, err = ExecuteSafeCommand("test-req", "cat", []string{"../../etc/passwd"})
	if err == nil || !strings.Contains(err.Error(), "Path traversal (..)") {
		t.Errorf("Expected path traversal error, got: %v", err)
	}

	// 3. Absolute Paths
	_, err = ExecuteSafeCommand("test-req", "ls", []string{"/var/log"})
